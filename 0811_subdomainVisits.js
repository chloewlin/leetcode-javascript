...
var subdomainVisits = function(cpdomains) {
  var countValidDomains = {};
  var result = [];

  // O(n) - n is the length of the input array
  for (var i = 0; i < cpdomains.length; i++) {
    var cpdomain = cpdomains[i];
    var visits = parseInt(cpdomain.split(" ")[0]);
    var domains = cpdomain.split(" ")[1];
    
    // Count the full domians
    countValidDomains[domains] = (countValidDomains[domains] || 0) + visits;

    // O(m) - m is the number of characters in each domain strings
    // Count all the possible subdomains 
    for (var j = 0; j < domains.length; j++) {
      if (domains[j] === '.') {
        var domain = domains.slice(j + 1);
        countValidDomains[domain] = (countValidDomains[domain] || 0) + visits;
      }
    }
  }

  for (var key in countValidDomains) {
    result.push(countValidDomains[key] + " " + key);
  }
